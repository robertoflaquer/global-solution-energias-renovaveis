    ...
    while True:
        try:
            ...
            if {"timestamp", "consumo_kwh"}.issubset(df.columns):
                return df
            print("CSV inválido. Faltam colunas necessárias.")
...
def detectar_desperdicios(df, limite_madrugada=40, limite_zscore=3):
    ...
    desvio = df['consumo_kwh'].std() if df['consumo_kwh'].std() != 0 else 1
...
def aplicar_otimizacao(df):
    reducoes = { "madrugada":0.25, "manha":0.10, "expediente":0.05, "noite":0.15 }
...
def calcular_economia(df_o, df_ot):
    ...
    economia = o - ot
    return o, ot, economia, economia / o * 100
